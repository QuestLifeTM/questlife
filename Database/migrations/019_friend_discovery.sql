-- Keep the canonical Database migration set in sync with the deployed
-- Supabase migration for the Social discovery experience.
--
-- The full, deployable migration lives at:
-- supabase/migrations/20260715100000_friend_discovery.sql
...
begin
  if current_user_id is null then raise exception 'Not authenticated'; end if;
  if coalesce(array_length(p_emails, 1), 0) = 0 then return '[]'::jsonb; end if;
  return coalesce((
    select jsonb_agg(row) from (
      select public.friend_discovery_profile(prof.id) as row
      from public.profiles prof
