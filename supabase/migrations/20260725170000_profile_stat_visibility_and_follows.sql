-- Lets each person choose which profile-stat pills friends can see.
alter table public.profiles
  add column if not exists stat_visibility jsonb not null default jsonb_build_object(
    'highestStreak', true,
    'level', true,
    'questsDone', true,
    'timeSpent', true,
...
-- Follow relationships are separate from mutual friendships. This keeps the
-- two new counts accurate without changing existing friend behaviour.
...
-- No direct client access is needed yet: the overview RPC below only exposes
-- aggregate counts. Keeping the follow graph private avoids leaking it.
...
begin
  ...
  select coalesce(prof.stat_visibility, jsonb_build_object(
    'highestStreak', true, 'level', true, 'questsDone', true, 'timeSpent', true,
    'totalXp', true, 'followers', true, 'following', true
  )) into stat_visibility
  from public.profiles prof where prof.id = target_user;
  ...
  select jsonb_build_object(
    ...
    'statVisibility', stat_visibility
  ...
  select coalesce(jsonb_agg(row order by (row->>'completedQuests')::integer desc, row->>'category'), '[]'::jsonb) into top_categories from (
    ...
    where completion.user_id = target_user
    group by quest.category
    order by count(*) desc, quest.category
    limit 3
  ) as category_rows;
  ...
  select jsonb_build_object(
    ...
    'totalQuestDurationSeconds', coalesce((select sum(extract(epoch from session.ended_at - session.started_at)::bigint) from public.quest_sessions session where session.user_id = target_user and session.status = 'completed' and session.ended_at is not null), 0) + coalesce((select sum(extract(epoch from completion.completed_at - completion.started_at)::bigint) from public.party_completions completion where completion.user_id = target_user), 0),
    'followers', (select count(*) from public.profile_follows where following_id = target_user),
    'following', (select count(*) from public.profile_follows where follower_id = target_user),
    'topCategories', top_categories
  ) into stats;

  if not is_self then
    if not coalesce((stat_visibility->>'highestStreak')::boolean, true) then stats := stats - 'longestStreak'; end if;
    if not coalesce((stat_visibility->>'questsDone')::boolean, true) then stats := stats - 'totalQuests'; end if;
    if not coalesce((stat_visibility->>'timeSpent')::boolean, true) then stats := stats - 'totalQuestDurationSeconds'; end if;
    if not coalesce((stat_visibility->>'totalXp')::boolean, true) then profile := profile - 'totalXp'; end if;
    if not coalesce((stat_visibility->>'followers')::boolean, true) then stats := stats - 'followers'; end if;
    if not coalesce((stat_visibility->>'following')::boolean, true) then stats := stats - 'following'; end if;
  end if;
