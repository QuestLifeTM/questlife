-- Quest posts need their own immutable completion duration so the feed can
-- render a post without reading a private journal or live quest record.
alter table public.quest_posts
  add column if not exists duration_seconds integer;

alter table public.quest_posts
  drop constraint if exists quest_posts_duration_seconds_check;

alter table public.quest_posts
  add constraint quest_posts_duration_seconds_check
  check (duration_seconds is null or duration_seconds >= 0);

create index if not exists quest_posts_visibility_created_idx
  on public.quest_posts (visibility, created_at desc);

-- p_scope = public exposes only deliberately-public posts.
-- p_scope = friends includes the viewer's friends' public/friends posts, plus
-- the viewer's own non-private posts. Personal journals never enter this feed.
