-- A first name may legitimately contain spaces. Return the stored field so
-- clients never need to guess name parts from the public display name.
  ...
  return coalesce((
    ...
    from (
      select jsonb_build_object(
        ...
        'displayName', coalesce(profile.display_name, 'Adventurer'), 'firstName', profile.first_name,
        'emoji', profile.emoji, 'avatarColor', profile.avatar_color, 'avatarUrl', profile.avatar_url,
