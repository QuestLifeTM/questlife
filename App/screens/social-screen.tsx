...
function PartyCreatedCelebration({ party, onShare, onContinue, onClose }: { party: { id: string; code: string; name: string } | null; onShare: () => void; onContinue: () => void; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!party) return;
    if (reduceMotion) { reveal.setValue(1); burst.forEach((value) => value.setValue(1)); return; }
    reveal.setValue(0);
        ...
        {step === "quests" && source === "custom" ? <>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            <View style={{ flex: 1, height: 52, borderRadius: 26, borderWidth: 2, borderColor: T.border, backgroundColor: T.white, paddingHorizontal: 15, flexDirection: "row", alignItems: "center", gap: 9, boxShadow: `3px 3px 0px ${T.border}` }}>
              ...
              <TextInput value={query} onChangeText={setQuery} placeholder="Search Quests" placeholderTextColor={T.muted} style={{ flex: 1, color: T.dark, fontWeight: "800", fontSize: 14, padding: 0 }} />
            </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 4 }}>
            {categories.map((item) => {
              ...
              return <Pressable key={item} accessibilityRole="button" accessibilityState={{ selected: active }} onPress={() => setCategory(item)} style={({ pressed }) => [{ borderRadius: 99, paddingVertical: 9, paddingHorizontal: 15, backgroundColor: active ? tone?.text ?? T.dark : tone?.bg ?? T.white, borderWidth: 2, borderColor: active ? tone?.text ?? T.dark : tone?.bg ?? T.border, boxShadow: active ? "none" : `2px 2px 0px ${T.border}`, transform: [{ translateY: pressed ? 1 : 0 }] }]}><Text style={{ color: active ? T.white : tone?.text ?? T.muted, fontSize: 12, fontWeight: "900" }}>{item}</Text></Pressable>;
            })}
          </ScrollView>
          ...
          {visibleQuests.map((quest) => {
            ...
            return <Pressable key={quest.id} accessibilityRole="checkbox" accessibilityState={{ checked: selected }} accessibilityLabel={`Add ${quest.title} to Party`} onPress={() => setSelectedQuestIds((prev) => selected ? prev.filter((id) => id !== quest.id) : [...prev, quest.id])} style={({ pressed }) => [{ flexDirection: "row", alignItems: "center", gap: 11, padding: 13, borderRadius: 18, backgroundColor: selected ? `${T.blue}12` : T.white, borderWidth: 2, borderColor: selected ? T.blue : T.border, borderBottomWidth: 5, borderBottomColor: selected ? "#258fd8" : T.border, transform: [{ translateY: pressed ? 2 : 0 }] }]}><View style={{ width: 36, height: 36, borderRadius: 13, backgroundColor: colors.bg, alignItems: "center", justifyContent: "center" }}><PartyCategoryIcon category={quest.category} size={20} /></View><View style={{ flex: 1, gap: 4 }}><Text style={{ color: T.dark, fontWeight: "900", fontSize: 16 }} numberOfLines={1}>{quest.title}</Text><View style={{ flexDirection: "row", alignItems: "center", gap: 7 }}><Text style={{ color: colors.text, fontSize: 10, fontWeight: "900", letterSpacing: 0.35, textTransform: "uppercase" }}>{quest.category}</Text><Text style={{ color: T.muted, fontSize: 11, fontWeight: "800" }}>+{quest.xp} XP · {quest.timeMin} min</Text></View></View><Ionicons name={selected ? "checkmark-circle" : "add-circle-outline"} size={21} color={selected ? T.blue : T.muted} /></Pressable>;
          })}
