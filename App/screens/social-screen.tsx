...
function SocialFeedLoading() {
  return <View accessibilityRole="progressbar" accessibilityLabel="Loading quest feed" style={{ width: "100%", gap: 16 }}>{[0, 1].map((index) => <View key={index} style={{ gap: 9 }}><View style={{ flexDirection: "row", alignItems: "center", gap: 9, paddingHorizontal: 3 }}><FeedSkeletonBlock width={36} height={36} radius={18} /><View style={{ flex: 1, gap: 5 }}><FeedSkeletonBlock width="38%" height={12} /><FeedSkeletonBlock width="24%" height={10} /></View></View><Card style={{ minHeight: index ? 162 : 224, borderRadius: 24, padding: 16, gap: 11 }}><FeedSkeletonBlock width="66%" height={20} /><FeedSkeletonBlock width="94%" height={12} /><FeedSkeletonBlock width="72%" height={12} />{index === 0 ? <FeedSkeletonBlock width="100%" height={86} radius={15} /> : null}</Card></View>)}</View>;
}
