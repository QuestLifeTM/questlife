...
export function QuestCollectionsScreen({ onBack }: { onBack: () => void }) {
  ...
  const alreadyFiled = useMemo(() => new Set(userPacks.flatMap((pack) => pack.questIds)), [userPacks]);
  const availableQuests = useMemo(() => quests.filter((quest) => quest.saved && !alreadyFiled.has(quest.id)), [alreadyFiled, quests]);
  ...
  const createCollection = async () => {
    ...
    try {
      ...
      await saveUserPack({ title: name.trim(), description: "Private quest collection", icon: "🧭", accentColor: T.blue, coverImageUrl, questIds: selectedQuestIds });
      reset();
