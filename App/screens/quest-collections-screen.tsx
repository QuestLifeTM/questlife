...
export function QuestCollectionsScreen({ onBack }: { onBack: () => void }) {
  ...
  const { userPacks, saveUserPack, loading: engineLoading } = useQuestEngine();
  ...
  return <Screen>
    ...
    {loading || engineLoading ? <CollectionGridSkeleton cardWidth={cardWidth} /> : userPacks.length ? visibleCollections.length ? <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 12 }}>{visibleCollections.map((pack) => <CollectionCard key={pack.id} pack={pack} quests={pack.questIds.map((id) => quests.find((quest) => quest.id === id)).filter(Boolean) as Quest[]} width={cardWidth} onOpen={() => router.push(`/collection/${pack.id}`)} onTogglePin={() => void togglePin(pack)} />)}</View> : <EmptyState emoji="🔎" title="No collections found" body="Try another collection name or a quest title you saved inside it." /> : <Card><EmptyState emoji="🗂️" title="Your collections live here" body="Create a collection to organize saved quests for a future adventure." action={<SoftButton label="Create collection" icon="add" color={T.blue} onPress={() => setStage("details")} />} /></Card>}
