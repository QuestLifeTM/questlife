...
function QuestHeaderPill({
  ...
  return <View style={{ minHeight: 29, paddingHorizontal: 10, borderRadius: 99, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 4, backgroundColor, borderWidth: 2, borderColor: color, borderBottomWidth: 3, borderBottomColor: `${color}88` }}>
    {icon}
...
export function QuestDetailScreen({ id, onBack }: { id?: string; onBack: () => void }) {
  ...
  return <Screen scroll={false} padded={false}>
    <View style={{ flex: 1 }}>
      ...
      <Reanimated.ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ ...edgePadding, paddingTop: 12, paddingBottom: 18, gap: 25 }}>
    ...
    <QuestStartBlockModal block={block} visible={Boolean(block)} onClose={clearBlock} onGoActive={() => { clearBlock(); if (engine?.activeSession) router.push("/active-quest"); }} onSaveActive={async () => { await saveActiveForLater(); clearBlock(); await refresh(); }} onRepeatQuest={async () => { if (block?.type !== "repeat_quest") return; const started = await tryStart({ questId: block.quest.id, source: "explore", confirmedRepeat: true }); if (started) { await refresh(); router.replace("/active-quest"); } }} />
