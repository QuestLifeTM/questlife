...
function PersonRow({ person, onAdd, onOpenProfile }: { person: ProfileSearchResult; onAdd: (person: ProfileSearchResult) => void; onOpenProfile: () => void }) {
  const status = person.isFriend ? "Friends" : person.requestStatus === "pending:outgoing" ? "Friend request sent" : null;
...
export function AddFriendsScreen() {
  ...
  async function addPerson(person: ProfileSearchResult) {
    try {
      ...
      const requested = { ...person, requestStatus: "pending:outgoing" };
      setSuggestions((items) => items.map((item) => item.userId === person.userId ? requested : item));
      setSearchResults((items) => items.map((item) => item.userId === person.userId ? requested : item));
      setContacts((items) => items.map((item) => item.userId === person.userId ? requested : item));
    } catch {
      Alert.alert("Couldn’t add friend", "Please try again in a moment.");
    }
  }
