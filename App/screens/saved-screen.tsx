...
export function SavedScreen({ onBack }: { onBack: () => void }) {
  ...
  return <Screen>
    ...
    <View style={{ gap: 12 }}><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}><Text style={{ color: T.dark, fontFamily: "RubikBlack", fontSize: 20 }}>Saved Quests</Text><Pressable onPress={() => router.push("/manage-saved")} hitSlop={8}><Text style={{ color: T.blue, fontFamily: "RubikBold", fontSize: 13 }}>Manage</Text></Pressable></View>
      {loading ? <SavedQuestListSkeleton /> : filtered.length ? filtered.map((quest) => <QuestRow key={quest.id} quest={quest} status={engine?.activeSession?.questId === quest.id ? "active" : quest.completed ? "completed" : undefined} onOpen={() => router.push(`/quest/${quest.id}`)} onRemove={() => void toggleSave(quest.id)} />) : <EmptyState emoji={search || activeFilterCount ? "🔍" : "📭"} title={search || activeFilterCount ? "No matches found" : "No unfiled saved quests"} body={search || activeFilterCount ? "Try adjusting your search, filters, or sorting." : "Quests added to a collection live inside that collection."} action={search || activeFilterCount || sortBy !== "Best Match" ? <SoftButton label="Reset all filters" icon="refresh" onPress={resetFilters} /> : <SoftButton label="Explore quests" icon="compass" color={T.blue} onPress={() => router.push("/(tabs)/explore")} />} />}
    </View>
