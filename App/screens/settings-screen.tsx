...
function SettingRow({ icon, color, title, detail, value, destructive = false, onPress, children }: { icon: IconName; color: string; title: string; detail?: string; value?: string; destructive?: boolean; onPress?: () => void; children?: React.ReactNode }) {
  const body = <View style={{ minHeight: 67, paddingHorizontal: 14, flexDirection: "row", alignItems: "center", gap: 11 }}><View style={{ width: 36, height: 36, borderRadius: 12, alignItems: "center", justifyContent: "center", backgroundColor: iconTint(color) }}><Ionicons name={icon} size={19} color={color} /></View><View style={{ flex: 1, minWidth: 0, gap: 2 }}><Text style={{ color: destructive ? T.red : T.dark, fontFamily: "RubikBold", fontSize: 15, lineHeight: 19 }} numberOfLines={1}>{title}</Text>{detail ? <Text style={{ color: T.muted, fontFamily: "Rubik", fontSize: 12, lineHeight: 16 }} numberOfLines={2}>{detail}</Text> : null}</View>{value ? <Text style={{ color: T.muted, fontFamily: "RubikBold", fontSize: 12 }} numberOfLines={1}>{value}</Text> : null}{children ?? (onPress ? <Ionicons name="chevron-forward" size={19} color={T.muted} /> : null)}</View>;
...
const notificationRows: ReadonlyArray<{ key: NotificationPreferenceKey; icon: IconName; color: string; title: string; detail: string }> = [
  { key: "streakAlerts", icon: "flame-outline", color: T.orange, title: "Streak Alerts", detail: "Get warned before your streak breaks" },
...
export function AppPreferencesScreen() {
  ...
  return <SettingsPage eyebrow="Account" title="Preferences"><SettingGroup title="App feel"><SettingRow icon="phone-portrait-outline" color={T.pink} title="Haptic Feedback" detail="Vibration on supported interactions"><Toggle value={settings.hapticFeedback} label="Haptic feedback" onChange={() => void updateHaptics()} /></SettingRow><Divider /><SettingRow icon="settings-outline" color={T.blue} title="Device permissions" detail="Manage notification, location, and photo access" onPress={() => { void Linking.openSettings().catch(() => undefined); }} /></SettingGroup></SettingsPage>;
}
