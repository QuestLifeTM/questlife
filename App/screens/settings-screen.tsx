...
import { IconButton, Screen, SoftButton, useResponsiveScreenLayout } from "@/components/ui";
...
function SettingsHeader({ eyebrow, title }: { eyebrow: string; title: string }) {
  ...
  return <View style={{ paddingHorizontal: 20, paddingBottom: 14, borderBottomWidth: 1.5, borderBottomColor: T.border }}><View style={{ minHeight: 76, flexDirection: "row", alignItems: "center", gap: 12 }}><IconButton icon="chevron-back" label="Back" onPress={() => router.back()} size={42} /><View style={{ gap: 2 }}><Text style={{ color: T.muted, fontFamily: "RubikBold", fontSize: 11, letterSpacing: 0.55, textTransform: "uppercase" }}>{eyebrow}</Text><Text style={{ color: T.dark, fontFamily: "RubikBlack", fontSize: 25, lineHeight: 30 }}>{title}</Text></View></View></View>;
}
