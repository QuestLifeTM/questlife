...
const JOURNAL_PROMPT_CHIPS = ["What surprised you?", "How did it make you feel?", "Would you do it again?"];
...
export function LogLoreFlow({ guestMode = false, visible, quest, onClose, onFinished, initialTitle, initialReflection = "", photoUris = [], durationSeconds = 0, onSaveDraft }: { guestMode?: boolean; visible: boolean; quest: Quest | null; onClose: () => void; onFinished: (result: CompletionResult, destination: CompletionDestination) => void | Promise<void>; initialTitle?: string; initialReflection?: string; photoUris?: string[]; durationSeconds?: number; distanceMeters?: number; onSaveDraft?: (draft: { title: string; body: string }) => Promise<void> }) {
  ...
  const save = async (visibility: "private" | "public" | "friends") => {
    ...
    if (!completion) try {
      ...
      if (guestMode) {
        completion = { completionId: `guest-${Date.now()}`, xpAwarded: 0, dailyUsed: 0, dailyLimit: 5 };
      } else {
        ...
        completion = await completeQuest({ questId: quest.id, logged: true, reflection: bodyForCompletion || null, rating, review: null, reviewPublic: false, photoUrls: journalPhotoPaths });
      }
    ...
    try {
      if (!guestMode && visibility !== "private") {
        ...
        await createQuestPost({ questId: quest.id, completionId: completion.completionId, title: quest.title, caption, photoUrls: uploaded, visibility, durationSeconds, stats: { rating, ...(composerShowsDuration ? { durationSeconds } : {}) } });
      }
    } catch (nextError) { setError(postPublishErrorMessage(nextError)); setPostRetryReady(true); setBusy(false); return; }
