...
function relativePostTime(value: string) { const timestamp = new Date(value); const elapsed = Date.now() - timestamp.getTime(); const hours = Math.floor(elapsed / 3_600_000); const time = timestamp.toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" }); if (hours < 24) return { relative: hours < 1 ? "Just now" : `${hours}h ago`, time }; if (hours < 48) return { relative: "Yesterday", time }; if (hours < 24 * 7) return { relative: `${Math.floor(hours / 24)}d ago`, time }; return { relative: timestamp.toLocaleDateString(undefined, { month: "long", day: "numeric", year: "numeric" }), time }; }
...
function firstName(value: string) { return value.trim().split(/\s+/)[0] || "Adventurer"; }
function displayProfileName(value: string, savedFirstName?: string | null) { const trimmed = value.trim(); return trimmed.length > 18 ? savedFirstName?.trim() || firstName(trimmed) : trimmed || savedFirstName?.trim() || "Adventurer"; }
...
function PostActionBar({ post, commentCount, onComments, onToggleReaction }: { post: QuestFeedPost; commentCount: number; onComments: () => void; onToggleReaction: () => Promise<void> }) {
  return <View style={{ minHeight: 58, paddingHorizontal: 16, borderTopWidth: 1, borderTopColor: T.border, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
    ...
    {post.commentsEnabled ? <Pressable accessibilityRole="button" accessibilityLabel="View comments" onPress={onComments} style={({ pressed }) => ({ minHeight: 38, borderRadius: 19, paddingHorizontal: 11, backgroundColor: "#fffdfa", borderWidth: 2, borderColor: T.border, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, opacity: pressed ? 0.72 : 1, transform: [{ scale: pressed ? 0.97 : 1 }] })}>
...
function FeedMedia({ post, loadMedia, actionBar }: { post: QuestFeedPost; loadMedia: boolean; actionBar: React.ReactNode }) {
  ...
  const description = post.caption?.trim() || `${displayProfileName(post.displayName, post.firstName)} did this quest.`;
...
export function QuestFeedCard({ post, loadMedia = true, onPostUpdated, onPostDeleted }: { post: QuestFeedPost; loadMedia?: boolean; onPostUpdated?: (post: QuestFeedPost) => void; onPostDeleted?: (postId: string) => void }) {
  ...
  const postedAt = relativePostTime(managedPost.createdAt);
